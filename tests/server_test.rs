...
use crt::protocol::{JsonRpcCall, JsonRpcMethod, NotificationKind};
...
impl ClientConn {
    ...
    async fn request_value(&mut self, request: &impl serde::Serialize) -> serde_json::Value {
...
async fn test_request_before_init() {
    ...
    let resp = conn
        .request(JsonRpcMethod::ListChangedFiles, serde_json::json!({}))
        .await;
...
async fn test_list_file_statuses_returns_compact_review_state() {
    ...
    let resp = conn
        .request(JsonRpcMethod::ListFileStatuses, serde_json::json!({}))
        .await;
...
async fn test_comment_lifecycle() {
    ...
    let detail = conn
        .request(JsonRpcMethod::GetComment, serde_json::json!({ "id": id }))
        .await;
...
async fn test_list_repos_reports_active_initialized_sessions() {
    ...
    let resp = discovery
        .request(JsonRpcMethod::ListRepos, serde_json::json!({}))
        .await;
    assert!(
        resp["error"].is_null(),
